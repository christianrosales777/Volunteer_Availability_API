...
const { dbIDLength, fromStart, toEnd, mIntervals } = require('../config/dbConfigs');
...
const updateVolunteerAvailability = async (req, res) => {
    if (!req?.body?.id || req.body.id.length !== dbIDLength) {
...
const getVolunteerAvailability = async (req, res) => {
    ...
    if (!volunteer) {
        return res.status(204).json({ 'message': `No volunteer matches the ID${req.params.id}` })
    }
